...
def to_excel(userlist, username):
    ...
    df = pd.DataFrame(columns=["ID", "title", "media type", "source", "mean score", "date_start", "date_finish",
                               "my score", "year", "season", "episodes", "episode duration"])
    dfgenres = pd.DataFrame(columns=["ID", "genres"])
    oldest = datetime.datetime.now().year
    seen = []
    for ulist in userlist["data"]["MediaListCollection"]["lists"]:

        for entry in ulist["entries"]:
            if entry["media"]["id"] not in seen:
                ...
                if None not in (entry['startedAt']['year'], entry['startedAt']['month'], entry['startedAt']['day']):
                    startdate = datetime.datetime(entry['startedAt']['year'], entry['startedAt']['month'],
                                                  entry['startedAt']['day'])
                else:
                    startdate = "Date not valid"
                if None not in (entry['completedAt']['year'], entry['completedAt']['month'], entry['completedAt']['day']):
                    completdate = datetime.datetime(entry['completedAt']['year'], entry['completedAt']['month'],
    ...
    workbook = mal_to_excel.resize_challenge_tables(workbook, genreslist)
